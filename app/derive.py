...
def run_rules(header: ECHeader, entries: list[Entry], edges: list[Edge]) -> list[Finding]:
    ...
    if start_year:
        ...
        if outside:
            add("R3", "blocking",
                ...
                f"period. The earliest is {min(outside, key=lambda t: t[1].year)[1].doc_no}. "
    ...
    if dangling:
        add("R4", "material",
            f"{len(dangling)} parent documents are named but not present in this "
            f"certificate: {', '.join(ed.to_doc_no for ed in dangling)}. "
            f"They have not been examined.",
